<x-app-layout>
    ...
    <x-slot name="header">
        Verifikasi Pengajuan (SSC)
    </x-slot>

    <x-slot name="toolbar">
        <form method="GET" action="{{ route('ssc.approval.index') }}" class="d-flex flex-column flex-md-row w-100 gap-3 align-items-md-center">
            <div class="toolbar-actions ms-md-auto d-flex flex-wrap gap-2 w-100 w-md-auto justify-content-start justify-content-md-end">
                <select name="status" class="form-select" onchange="this.form.submit()" style="min-width: 170px;">
                    ...
                    <option value="menunggu_ssc" @selected(request('status') === 'menunggu_ssc')>Menunggu SSC</option>
                    <option value="menunggu_logistik" @selected(request('status') === 'menunggu_logistik')>Diteruskan ke Logistik</option>
                        ...
                        @forelse ($reservasis as $reservasi)
                            <tr>
                                ...
                                <td class="py-3 text-end pe-4">
                                    <a href="{{ route('ssc.approval.show', $reservasi) }}" class="btn btn-sm btn-dark d-inline-flex align-items-center gap-2">
                            ...
                            <tr>
                                <td colspan="6" class="text-center py-5">
                                    <div class="teluroom-empty-state py-0">
                                        ...
                                        <p class="text-secondary mb-0" style="font-size: 0.875rem;">Tidak ada pengajuan yang perlu diverifikasi.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
