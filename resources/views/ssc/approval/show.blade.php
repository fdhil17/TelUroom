<x-app-layout>
    <x-slot name="header">
        <h2 class="fs-4 fw-bold text-dark">Tinjau Pengajuan Peminjaman</h2>
    </x-slot>
        ...
        <div class="row justify-content-center">
            <div class="col-lg-8">
                ...
                @if ($reservasi->status === 'menunggu_ssc')
                    <div class="card mb-4">
                        <div class="card-header fw-bold card-header-dark">Proses Verifikasi Pengajuan</div>
                        <div class="card-body p-4">
                            ...
                            <form method="POST" action="{{ route('ssc.approval.process', $reservasi) }}" id="formApproval">
                                @csrf
                                <input type="hidden" name="action" id="actionInput">

                                <div class="mb-4">
                                    <label for="catatan_ssc" class="form-label">Catatan <span class="text-muted fw-normal">(opsional jika setuju, wajib jika tolak)</span></label>
                                    <textarea name="catatan_ssc" id="catatan_ssc" rows="3"
                                        class="form-control @error('catatan_ssc') is-invalid @enderror">{{ old('catatan_ssc') }}</textarea>
                                    @error('catatan_ssc')
                                ...
                                <div class="d-flex justify-content-end gap-2">
                                    ...
                                    <button type="button" class="btn btn-success"
                                        data-bs-toggle="modal" data-bs-target="#confirmModal"
                                        data-action="approve"
                                        data-title="Verifikasi & Setujui"
                                        data-message="Apakah Anda yakin ingin menyetujui pengajuan ini?"
                ...
                @else
                    @if ($reservasi->catatan_ssc)
                        <div class="alert alert-info">
                            <strong>Catatan SSC:</strong> {{ $reservasi->catatan_ssc }}
                        </div>
                    @endif
                @endif

                <div class="mt-2">
                    <a href="{{ route('ssc.approval.index') }}" class="btn btn-secondary">← Kembali</a>
                </div>
