<x-app-layout>
    <x-slot name="header">
        <h2 class="fs-4 fw-bold text-dark">{{ __('dashboard.ssc_judul') }}</h2>
    </x-slot>
    ...
    <div class="container py-4">
        ...
        <div class="row mb-4">
            <div class="col-md-3 mb-3">
                <div class="card stat-card text-center stat-warning">
                    <div class="card-body">
                        <h3 class="fw-bold">{{ $statistik['menunggu_ssc'] }}</h3>
                        <p class="mb-0">{{ __('dashboard.menunggu_persetujuan') }} SSC</p>
                    </div>
                </div>
            </div>
            <div class="col-md-3 mb-3">
                <div class="card stat-card text-center stat-info">
                    <div class="card-body">
                        <h3 class="fw-bold">{{ $statistik['menunggu_logistik'] }}</h3>
                        <p class="mb-0">{{ __('dashboard.diteruskan_ke_logistik') }}</p>
                    </div>
                </div>
            </div>
            <div class="col-md-3 mb-3">
                <div class="card stat-card text-center stat-success">
                    <div class="card-body">
                        <h3 class="fw-bold">{{ $statistik['disetujui'] }}</h3>
            ...
            <div class="col-md-3 mb-3">
                <div class="card stat-card text-center stat-danger">
                    <div class="card-body">
                        <h3 class="fw-bold">{{ $statistik['ditolak_ssc'] }}</h3>
                        <p class="mb-0">{{ __('dashboard.ditolak') }} SSC</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card">
            <div class="card-header fw-bold card-header-dark">{{ __('dashboard.pengajuan_terbaru_semua') }}</div>
            <div class="card-body">
                        ...
                        <tbody>
                            ...
                            @foreach ($reservasiTerbaru as $r)
                                <tr>
                                    ...
                                    <td class="text-end">
                                        <a href="{{ route('ssc.approval.show', $r) }}" class="btn btn-sm btn-outline-primary">{{ __('pengajuan.btn_detail') }}</a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
                ...
                <a href="{{ route('ssc.approval.index') }}" class="btn btn-outline-primary mt-2">{{ __('dashboard.lihat_semua_verifikasi') }}</a>
            </div>
        </div>
